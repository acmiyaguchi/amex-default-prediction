...
def test_arrow_data_module_has_fields(synthetic_train_data_torch_path):
    batch_size = 4
    data_module = ArrowDataModule(
        synthetic_train_data_torch_path, batch_size=batch_size
    )
    data_module.setup()
    dataloader = data_module.train_dataloader()
    batches = 0
...
def test_trainer_accepts_arrow_data_module(synthetic_train_data_torch_path):
    data_module = ArrowDataModule(synthetic_train_data_torch_path, batch_size=4)
    trainer = pl.Trainer(fast_dev_run=True)
    model = StrawmanNet(input_size=3)
    trainer.fit(model, datamodule=data_module)
